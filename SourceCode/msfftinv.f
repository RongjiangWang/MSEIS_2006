...
c
      ...
      enddo
c
      print *,' Output: '//outfile(icmp)(1:flen(icmp))
c
...
c
        do lf=1,nf
          do ir=1,nr
            s=dcmplx(-fi,f(lf))
     &       *dcmplx(pi2*(tstart+dsqrt(zrs2+r(ir)*r(ir))*slw0),0.d0)
            cy(lf,ir)=grns(lf,icmp,ir)*cdexp(s)
          enddo
        enddo
      else
        do lf=1,nf
          do ir=1,nr
            cy(lf,ir)=grns(lf,icmp,ir)
          enddo
        enddo
      endif
c
...
c
      ...
      enddo
c
      open(20,file=outfile(icmp),status='unknown')
      write(20,'(a,$)')'   T_sec    '
      do ir=1,nr-1
